  ...
  return `
    <html>
      <body style="font-family: sans-serif; padding: 20px;">
        ...
        <h2>Diagrama de ferrocarril:</h2>
        <img 
          src="${imageUri}" 
          style="width: 100%; max-width: 600px; max-height: 400px; object-fit: contain;" 
        />
      </body>
    </html>
  `;
...
export const RegexTesterScreen: React.FC = () => {
  ...
  const handleExportToPdf = async () => {
    const imageBase64 = await diagramRef.current?.capture?.();
    try {
      if (!imageBase64) {
        Alert.alert('Error', 'No se pudo capturar el diagrama.');
        return;
      }
      const imageUri = `data:image/png;base64,${imageBase64}`;

      const matchTexts = matches.map(([start, end]) => testText.slice(start, end));
      const html = generatePdfContent(regex, testText, matchTexts, imageUri);
